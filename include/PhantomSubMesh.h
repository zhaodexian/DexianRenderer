...
class	 PHANTOM_API RenderBase: public Base//渲染物体基础接口
{
public:
...
class	RenderNode;
class	 PHANTOM_API RenderNodeManager
{
public:
	RenderNodeManager();
...
public:
	TypeArray<RenderNode*>	m_renders;//渲染对象
};

class	 PHANTOM_API RenderNode: public Base
{
public:
	ImplementComValue();
...
class  PHANTOM_API VertexAnimationMesh
{
public:
...
class	 PHANTOM_API MeshMaterial: public MeshMaterialAttribute
{
public:
	MeshMaterial();
	~MeshMaterial();
...
public:
	...
	TypeArray<Texture*>					m_uvmaps;//UV动画中的图片列表，此项跟着第一个图片的载入会全部被自动载入，必须同时存在两个条件，第一个条件是图片必须uvmap_开头，第二个是这个图片对应的.txt文件中列出所有uv动画序列
	char								m_uvmapLoop;
	float								m_uvmapSpeed;
...
class  PHANTOM_API SubMeshPolygon: public Base
{
public:
	SubMeshPolygon();
...
class  PHANTOM_API SubMeshData: public Base
{
public:
	SubMeshData();
...
class  PHANTOM_API SubMesh: public RenderBase
{
public:
	SubMesh();
...
class	 PHANTOM_API MeshShaderProject
{
public:
	MeshShaderProject();
