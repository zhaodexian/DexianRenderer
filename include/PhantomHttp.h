...
#define	jval(var,name)	var[#name]
	...
	class PHANTOM_API HttpRequest: public ThreadProc
	{
	public:
		HttpRequest();//构造函数
		...
		TypeArray<char>				m_recvBuffers;
		int								m_jsonCount;
		BOOL							m_bFail;
		PhantomLock						m_lock;
		BOOL							m_bRequestComplete;
	};
	class	PHANTOM_API AuthHttpRequest: public HttpRequest, public IJsonAnalyseCallback
	{
	public:
		AuthHttpRequest();
		~AuthHttpRequest();
	...
	PHANTOM_API VOID			RequestIP(fnHttpCallback cb, JsonObject& params, const char* szHome, void* param);
	PHANTOM_API void			SetServerIP(const char* ip);
	PHANTOM_API VOID			Request(const char* infile, JsonObject& params, fnHttpCallback cb, BOOL bUseThread = false, void* param = 0);//请求数据
	///
	PHANTOM_API VOID			OnDownloadFrame();
	PHANTOM_API char*			urldecode(char* inOut);//对文本进行反编
	PHANTOM_API bool			urlencode(char* ret, int bufferLength, const char* sIn, int len = 0);
	//下载文件
	typedef	VOID	(*PDownloadComplte)(const char* requestText, const char* indocFile, BOOL bError, char* buffer, int length);//bError是否有错误
	PHANTOM_API BOOL			downloadFile(const char* requestText, const char* indocFile,PDownloadComplte callback = 0, BOOL bUseThread = true, BOOL bInMemory = false);//从网络下载文件
};
#endif
