...
#pragma pack(pop)
	...
	public:
		...
		virtual	bool			OnInputEvent(Phantom::InputEventID evt, int x, int y, unsigned int touchIndex) { return false; }
		//
		virtual	BOOL			GotoBack(){return false;}//是否可以返回
	public:
	};
	typedef	classPtr<MyFont>		FontPtr;
	...
	public://engine
		...
		virtual	bool				AppInputEvent(Phantom::InputEventID btn, int x, int y, unsigned int touchIndex);
		//
	...
	public://creates
		...
		Texture*					LoadTexture(const char* szFile, BOOL bGenMipmap = false, int nFilter = TextureFilter_Linear, BOOL bLoadThread = false, BaseDirectory* basedir = 0);
		Texture*					CreateTexture(int nWidth, int nHeight, int nFormat = ImageBuffer::PixelFormat_RGBA);
		//
	...
	public://2D draws, x, y, (z must be 0), lookat z
		...
		VOID						RenderText2D(DrawBuffer* buffer, RenderText* textPtr, const RectF& rc, const color4& c, const RectF* TextRect = 0, bool bBack = false);
		void						RenderTexture(DrawBuffer* buffer, Texture* t, const RectF& screen, const RectF& texRc, const color4& c, bool bEnabled);
		...
		//
		BOOL						DrawTexts(const char* szUTF8, Rect& destRect, int nLength = 0, float fontSize = 16.0f, float fItalic = 0, const color4* c = 0, unsigned int dtFormat = DrawTextAlign_LEFT | DrawTextAlign_TOP, const matrix4x4* world = 0, bool bWithBack = false);	//
		BOOL						DrawTexts(RenderText* textPtr, BOOL bEnabled, Rect& destRect, unsigned int dtFormat, const color4& color, const matrix4x4* world = 0, int nHighlight = 0, bool bWithBack = false);
		...
		inline	MyFont*				GetDefaultENFont(){return m_defaultENFont;}
		inline	MyFont*				GetDefaultUnicodeFont(){return m_defaultUnicodeFont;}
	...
	protected://shader modes
		...
		//
		Size						m_uiDialogSize;
		AutoPtr<MyFont>				m_defaultENFont;
		AutoPtr<MyFont>				m_defaultUnicodeFont;	//中文字体
