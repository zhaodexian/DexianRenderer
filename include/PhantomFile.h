...
class	PHANTOM_API TableTxt: public ITable
{
public:
	TableTxt(void);
	~TableTxt(void);
...
public:
	...
	static char					ExportPak(const char* szFile, TypeArray<PakFileData*>& datas);//, const unsigned char* pass, int passlen);
	//
	char						LoadPakFile(CSafeFileHelperR& r, PakFileInfo& info, TypeArray<char>& rets);
	static BOOL					ResetPassword(TypeArray<char>& dest, TypeArray<char>& src, const unsigned char* oldpass, int oldpasslen);
	//
...
public:
	...
	TypeArray<PakFileInfo>	m_pakfiles;
};
