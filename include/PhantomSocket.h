...
namespace Phantom{
	...
	PHANTOM_API 	BOOL		SetLastSockError(DWORD dwError);
	PHANTOM_API 	DWORD		GetLastSockError();
	PHANTOM_API 	const char*	GetLastSockErrorText();
	PHANTOM_API 	void		Block(Socket s, BOOL bBlock);//设置是否进入阻塞模式
	//
	class	NetToAI;
	//网络钩子接口
	class	NetAICB
...
#pragma pack(pop)
	...
	struct PHANTOM_API PhantomLock
	{
		PhantomLock();
		~PhantomLock();
		VOID				Lock();
	...
	PHANTOM_API VOID			createThread(PThreadCallback cb, void* parameter);
	//线程控制类
	class	ThreadProc;
	class	 PHANTOM_API Thread
	{
	public:
		Thread();
		virtual ~Thread(){Stop();}
	...
	class	 PHANTOM_API ThreadProc: public Base
	{
	public:
		ImplementComValue();
		ThreadProc(){}
	...
	class	 PHANTOM_API NetRecvBase: public ThreadProc
	{
	public:
		NetRecvBase();
		virtual	~NetRecvBase(){};
	...
	class  PHANTOM_API NetRecvMessage: public NetRecvBase
	{
	public:
		NetRecvMessage();
		~NetRecvMessage(){}
		...
		static TypeArray<unsigned char>		s_recvList[2];		//接收到的消息
		static TypeArray<unsigned char>*	s_recvCurrent;
	...
	class  PHANTOM_API NetClient: public NetRecvMessage
	{
	public:
		NetClient();//构造函数
	...
	class	NetServer;
	//网络钩子接口
	class  PHANTOM_API NetServerLink: public NetRecvBase
	{
	public:
		virtual	VOID				Close();
	...
	class  PHANTOM_API NetServer: public NetRecvMessage
	{
	public:
		NetServer();//构造函数
	...
	class	AIControls;
	class	 PHANTOM_API NetToAI: public ThreadProc//发送给AI控制器
	{
	public:
		NetToAI();//构造函数
	...
	protected:
		TypeArray<PackageAI>			m_recvs[2];		//接收到的消息
		TypeArray<PackageAI>*			m_current;
		NetClientInfo					m_info;
		NetAICB*						m_callback;
		PhantomLock						m_lock;
	protected:
	...
	//
	class	 PHANTOM_API AIControls: public ThreadProc//AI控制器，所有AI都在这里控制
	{
	public:
		AIControls();
		virtual ~AIControls(){}
	...
	PHANTOM_API BOOL						Send(DWORD linkid, WORD wID, const char* dataPtr, unsigned int wSize);//一次最多可以发送8190个字节，发送到某个连接,wID必须小于30000且大于0
	PHANTOM_API VOID						CloseWait(DWORD linkid, INT nMillisecond);//设置关闭时间
	PHANTOM_API NetRecvBase*				GetRecvByID(DWORD linkid);//获取某个连接的指针
	PHANTOM_API VOID						CloseLink(DWORD linkid);//关闭某个连接
	PHANTOM_API VOID						StopAllSocket();//关闭所有的服务器跟客户端
	PHANTOM_API VOID						SetupSocket(FnLink link, FnDislink dislink, FnMessage message, DWORD dwGUID);//初始化网络模块
	PHANTOM_API BOOL						StartServer(const char* name, WORD wPort);//开启服务器
	PHANTOM_API DWORD						StartClient(const char* szIP, WORD wPort, FnLink link, FnDislink dislink, FnMessage message, DWORD dwGUID);//开启客户端，返回一个客户端唯一linkid
	PHANTOM_API BOOL						SearchServers(FnSearchComplete fn, WORD wPort);//搜索所有的网段内的服务器，搜索完毕之后调用
	extern	PHANTOM_API NetServer*			g_server;
};
