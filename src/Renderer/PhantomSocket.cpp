...
namespace Phantom{
	...
	{
		DoSearchMessage();
		TypeArray<unsigned char>* doRecv	=	0;
		{
			Lock l(s_recvLock);
	...
	TypeArray<unsigned char>	NetRecvMessage::s_recvList[2];		//接收到的消息
	TypeArray<unsigned char>*	NetRecvMessage::s_recvCurrent = &s_recvList[0];
	...
	VOID			NetRecvMessage::RecvBuffer(unsigned char type, WORD wMessageID, void * pData, unsigned int wDataSize, SocketID* id)	//接收消息
	{
		...
		s_recvCurrent->resize(oldSize+wDataSize+sizeof(Package));
		//if(s_recvCurrent->counts >= s_recvCurrent->alloced)
		//{
		//	s_recvCurrent->alloced += 100;
		//	s_recvCurrent->recvs.resize(s_recvCurrent->alloced);
		//}
		{
