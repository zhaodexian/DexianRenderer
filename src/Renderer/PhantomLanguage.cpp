...
#ifdef WIN32_OS
PHANTOM_API VOID	ReplaceFromTable(wchar_t* buffer, const wchar_t* szEnText)
{
	while(*szEnText != 0)
	{
		if(*szEnText == L'\t')
...
PHANTOM_API VOID	ReplaceToTable(wchar_t* buffer, const wchar_t* szEnText)
{
	while(*szEnText != 0)
	{
...
PHANTOM_API wchar_t*	ConvertUnicodeAB(wchar_t* ptr)
{
	wchar_t* ori	=	ptr;
	while(*ptr != NULL)
	{
...
PHANTOM_API VOID	ReplaceToTable(char* buffer, const char* szEnText)
{
	while(*szEnText != 0)
	{
