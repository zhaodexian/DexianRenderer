...
namespace Phantom{
	...
	struct	PngReader
	{
		...
		BOOL		Load(ImageBuffer& ret, const void* dataPtr, int dataLength, BOOL bDivUsage)
		{
			...
			png_read_png(structPtr, infoPtr, PNG_TRANSFORM_EXPAND, 0);// | PNG_TRANSFORM_STRIP_16 | PNG_TRANSFORM_GRAY_TO_RGB, 0);
			int colorType;
			int nBPP;
...
/*
	...
	BOOL				RenderTextFig::ResetFontFig(RenderTextFig::RTFigRect& fig, short_t ch){CPUTime(RenderTextFig);
		...
		MyFont* font	=	g_manager.GetDefaultENFont();
		if(!font)
			return false;
		MyFont::FontTextInfo* info	=	font->GetTextInfo(ch);
		fig.fontEn = true;
		if(!info)
		{
			MyFont* cfont	=	g_manager.GetDefaultUnicodeFont();
			if(cfont)
			{
	...
	{CPUTime(Texture);
		...
		if (m_isCreate) {
			...
			//this->m_nCreateWidth = nWidth;
			//this->m_nCreateHeight = nHeight;
			//this->m_nCreateFormat = nFormat;
			return true;
		}
#ifndef EXPORT_ENGINE
		m_imgBuffer.FreePixels();
	...
	unsigned char*	Texture::Lock()
	{
		if (!m_isCreate)return 0;
		return m_imgBuffer.pixels;
	}
	void			Texture::Unlock()
	{
		if (m_isCreate) {
	...
	BOOL			Texture::Create(int nWidth, int nHeight, int nFormat)
	{
		this->m_isCreate = true;
		m_imgBuffer.width = nWidth;
		m_imgBuffer.height = nHeight;
		int bytesPerPixel = 3;
		m_imgBuffer.bpp = nFormat;
		if (m_imgBuffer.bpp == ImageBuffer::PixelFormat_RGBA)
			bytesPerPixel = 4;
		m_imgBuffer.pixels = (unsigned char*)malloc_s(nWidth * nHeight * bytesPerPixel * sizeof(unsigned char));
		//memset(m_imgBuffer.pixels, 0, nWidth * nHeight * bytesPerPixel * sizeof(unsigned char));
		//为什么设置像素显示不出来
		if (m_imgBuffer.bpp == ImageBuffer::PixelFormat_RGBA) {
			RGBA* rgba = (RGBA*)m_imgBuffer.pixels;
			RGBA r;
			r.r = r.g = r.b = 0;
			r.a = 255;
			for (int x = 0; x < m_imgBuffer.width; x++) {
				for (int y = 0; y < m_imgBuffer.height; y++) {
					rgba[x + y*m_imgBuffer.width] = r;
				}
			}
		}
		m_imgBuffer.oriwidth = nWidth;
		m_imgBuffer.oriheight = nHeight;
		if (g_manager.IsDeviceSetup())
		{
			OnResetDevice();
		}
