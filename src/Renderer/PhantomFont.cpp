...
namespace Phantom{
	...
	BOOL							MyFont::LoadFont(const char* szFigFile)
	{CPUTime(MyFont);
		CFileHelperR r;
		if(!r.open(szFigFile))
		{
	...
	BOOL							MyFont::DrawTexts(const char* szUTF8, Rect& destRect, unsigned int len, unsigned dtFormat, const matrix4x4* world)
	{CPUTime(MyFont);
		if(len == 0)
			len = strlen(szUTF8);
	...
	BOOL							MyFont::DrawTextClip(const short_t* szU, Rect& destRect, unsigned int len, unsigned dtFormat, const matrix4x4* world, const Rect* clip, BOOL bIsUI)
	{CPUTime(MyFont);
	...
	BOOL							MyFont::DrawTexts(const short_t* szU, Rect& destRect, unsigned int len, unsigned dtFormat, const matrix4x4* world, const rect_f* scale, BOOL bIsUI)
	{CPUTime(MyFont);
	...
	MyFont::FontTextInfo*		MyFont::GetTextInfo(unsigned short u)
	{CPUTime(MyFont);
	...
	int								MyFont::GetTextDrawCount(const short_t* szUnicode, int textCount, unsigned int nPixelWidth)
	{CPUTime(MyFont);
	...
	VOID							MyFont::SetFontSize(float f)
	{CPUTime(MyFont);
		m_fFontSize = f;
		if(m_fFontSize < 0)
	...
	Size							MyFont::GetTextDrawSize(const short_t* szUnicode, int textCount)
	{CPUTime(MyFont);
		Rect destRect(0, 0, g_manager.GetBufferWidth(), g_manager.GetBufferHeight());
	...
	BOOL							MyFont::DrawUnicode(short_t u, rect_f& dio, float& maxHeight, const Rect& destRc, BOOL bDraw, float& maxLineHeight, float2* topSpace, BOOL bIsUI, const Rect* clip)
	{CPUTime(MyFont);
		if(u == ' ')
		{
			u = '_';
	...
	BOOL			MyFont::InitDrawText(const Rect& rc, const color4* c, const matrix4x4* world, const rect_f* scale)
	{CPUTime(MyFont);
	...
	VOID			MyFont::DrawUnicode(Texture* tex, int channel, const rect_f& screen, const rect_f& texRc, BOOL bIsUI)	//渲染2D图片
	{CPUTime(MyFont);
		assert(channel < 3);
