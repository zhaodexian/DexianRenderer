...
namespace Phantom{
	...
	{CPUTime(CMapObject);
		...
		if(inter->GetOriginal().x < m_rect.left || inter->GetOriginal().y < m_rect.top || inter->GetOriginal().x > m_rect.right || inter->GetOriginal().y > m_rect.bottom)
			return false;
		return false;// m_mesh->GetHeight(inter, &m_mat);
	}
	...
	VOID			Map3DQuadBox::Intersects(TypeArray<CMapObject*>& objs, IntersectData* intersect)
	{CPUTime(Map3DQuadBox);
		if(this->m_nChildObjectCount <= 0)
			return;
		float3& _min		=	m_regionBox._min;
	...
	VOID			Map3DQuadBox::SelectObjects(TypeArray<CMapObject*>& objs, const Rect& rc)
	{CPUTime(Map3DQuadBox);
		if(this->m_nChildObjectCount <= 0)
			return;
		CMapObject* ret = 0;
	...
	{CPUTime(Map3D);
		...
		if(version<=1)//载入早期版本
		{
			...
			int numTrigger = 0;
			r >> numTrigger;
			this->m_triggers.resize(numTrigger);
		...
		TypeArray<float3> normals;
		normals.resize((m_mapSize.cx+1)*(m_mapSize.cy+1));
