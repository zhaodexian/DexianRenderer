...
namespace Phantom{
	...
	void					Manager::SetClearColor(const color4& c)
	{
	...
	{CPUTimeT(Manager);
		...
		CreateProgramFromMode(RenderMode_ImageBufferDisable, "default_init_zip/shader/tech_imagebuffer_disable.inl");
		CreateProgramFromMode(RenderMode_Text, "default_init_zip/shader/tech_text.inl");
		CreateProgramFromMode(RenderMode_TextWithBack, "default_init_zip/shader/tech_text_withback.inl");
...
//#endif
		...
		if(m_bRenderUI)
		{
			...
			if(m_bShowFPS)
			{
				...
				DrawTexts(buf, screen, 0, 16, 0, &r, DrawTextAlign_CENTER | DrawTextAlign_TOP, 0, true);
			}
		}
	...
	VOID					Manager::RecalcRenderRect()
	{
		...
		m_progMode[RenderMode_ImageBufferDisable]->SetFloat4(varUVSO, float4(m_uiScale.x, m_uiScale.y, m_uiOffset.x, m_uiOffset.y), "uvSO");
		m_progMode[RenderMode_ImageBufferDisable]->SetFloat2(varR, float2(fX, fY), "R");
	...
	bool							Manager::AppInputEvent(Phantom::InputEventID btn, int x, int y, unsigned int touchIndex)
	{CPUTimeT(Manager);
	if (!this->m_bIsInitialized)return false;
		Lock lock(m_lockRender);
		//
		...
		else
		{
			...
			if(this->GetActiveSDK())
				return GetActiveSDK()->OnInputEvent(btn, x, y, touchIndex);
		}
		return false;
	}
	...
	{CPUTimeT(Manager);
		...
		int id = 0;
		MyFont* v	=	m_fonts.addNew(&id, new classT<MyFont, classMgr<MyFont> >());	//添加自动删减功能
		if(!v->LoadFont(f.t))
		{
			safe_release(v);
			return false;
	...
	Texture* Manager::LoadTexture(const char* szFile, BOOL bGenMipmap, int nFilter, BOOL bLoadThread, BaseDirectory* basedir)
	{
		Texture* tex = 0;
		if (!LoadTexture(&tex, szFile, bGenMipmap, nFilter, bLoadThread, basedir))
			return 0;
		return tex;
	}
	Texture* Manager::CreateTexture(int nWidth, int nHeight, int nFormat)
	{
		int id = 0;
		Texture* v = m_textures.addNew(&id, m_device->CreateTexture());	//添加自动删减功能
		if (!v->Create(nWidth, nHeight, nFormat))
		{
			safe_release(v);
			return 0;
		}
