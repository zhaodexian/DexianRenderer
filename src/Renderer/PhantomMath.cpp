...
namespace Phantom{
	...
	PHANTOM_API float		planeDot( const Plane *pP, const float4 *pV)
	{
		return pP->a * pV->x + pP->b * pV->y + pP->c * pV->z + pP->d * pV->w;
	}
	PHANTOM_API float		planeDotCoord( const Plane *pP, const float3 *pV)
	{
		return pP->a * pV->x + pP->b * pV->y + pP->c * pV->z + pP->d;
	}

	PHANTOM_API float		planeDotNormal( const Plane *pP, const float3 *pV)
	{
		return pP->a * pV->x + pP->b * pV->y + pP->c * pV->z;
	}

	PHANTOM_API Plane*		planeScale(Plane *pOut, const Plane *pP, float s)
	{
		pOut->a = pP->a * s;
		pOut->b = pP->b * s;
		pOut->c = pP->c * s;
	...
	PHANTOM_API Plane*				planeFromPointNormal( Plane *pOut, const float3 *pPoint, const float3 *pNormal)
	{
		pOut->a = pNormal->x;
		pOut->b = pNormal->y;
		pOut->c = pNormal->z;
	...
	PHANTOM_API Plane::Plane( const float3& p1, const float3& p2, const float3& p3 )
	{
		float3 m = p2-p1;
		float3 n = p3-p1;
	...
	//从ARGB值转换成颜色数据
	PHANTOM_API color4			color4::fromRGBA(unsigned char R, unsigned char G, unsigned char B, unsigned char A)
	{
		color4 result;
	...
	PHANTOM_API color4*  colorAdjustContrast(color4 *pout, const color4 *pc, float s)
	{
	...
	PHANTOM_API color4*  colorAdjustSaturation(color4 *pout, const color4 *pc, float s)
	{
		float grey;
	...
	PHANTOM_API matrix4x4&			matrix4x4::operator	=	(const matrix4x4& v)
	{
		memcpy_s(this,&v,sizeof(matrix4x4));
		return *this;
	}
	//将弧度限制范围在 -PI ~ PI 之间
	PHANTOM_API float					clampDegress(float fDegress)
	{
	...
	PHANTOM_API matrix4x4*	matrixMultiply(matrix4x4 *pOut, const matrix4x4 *pM1, const matrix4x4 *pM2)
	{CPUTime("matrix4x4");
#if defined(WIN32_OS) && defined(IN_ENGINE)
...
//        return (matrix4x4*)matrixMultiply2((float*)pOut, (float*)pM1, (float*)pM2);
	...
	PHANTOM_API matrix4x4* matrixLookAtRH( matrix4x4 *pOut, const float3 *pEye, const float3 *pAt,const float3 *pUp )
	{CPUTime("matrix4x4");
		float3 eye = *pEye;
		float3 at = *pAt;
		float3 up = *pUp;
	...
	PHANTOM_API matrix4x4* matrixPerspectiveFovRH( matrix4x4 *pOut, float fovy, float Aspect, float zn, float zf )
	{CPUTime("matrix4x4");
	...
	PHANTOM_API float				CalculateDeterminant3( float _11, float _12, float _13, float _21, float _22, float _23, float _31, float _32, float _33)
	{
		return _11 * (_22 * _33 - _23 * _32) - _12 * (_21 * _33 - _23 * _31) + _13 * (_21 * _32 - _22 * _31);
	}
	PHANTOM_API float				Determinant(const matrix4x4& m)
	{
	...
	PHANTOM_API matrix4x4*	matrixScaling( matrix4x4 *pOut, float x, float y, float z )
	{CPUTime("matrix4x4");
	...
	PHANTOM_API matrix4x4*	matrixInverse( matrix4x4 *pOut, float *pDeterminant, const matrix4x4 *pM )
	{CPUTime("matrix4x4");
#if defined(WIN32_OS) && defined(IN_ENGINE)
	...
	PHANTOM_API matrix4x4*	matrixOrthoRH(matrix4x4* m, float w,float h,float zn,float zf)
	{CPUTime("matrix4x4");
		float nfRatio;
	...
	PHANTOM_API matrix4x4*	matrixRotationX(matrix4x4* m, float f)
	{CPUTime("matrix4x4");
		float s = (float)sin(f);	
		float c = (float)cos(f);	
	...
	PHANTOM_API matrix4x4*	matrixRotationY(matrix4x4* m, float f)
	{CPUTime("matrix4x4");
		float s = (float)sin(f);	
		float c = (float)cos(f);	
	...
	PHANTOM_API matrix4x4*	matrixRotationZ(matrix4x4* m, float f)
	{CPUTime("matrix4x4");
		float s = (float)sin(-f);
		float c = (float)cos(-f);
	...
	PHANTOM_API float3&	TransformRot(float3& dest, float3& v, matrix4x4& matWorld)
	{CPUTime("matrix4x4");
	...
	PHANTOM_API float			GetVec3Radius(float3& v){return sqrtf(v.x*v.x+v.y*v.y+v.z*v.z);}
	...
	PHANTOM_API float3* vec3Transform( float3 *pDest, const float3 *pV, const matrix4x4 *pM )
	{
	...
	PHANTOM_API BOOL		IntersectTriangle(const Pixel& cursor, const Pixel& pv0, const Pixel& pv1, const Pixel& pv2)
	{CPUTime("matrix4x4");
	...
	PHANTOM_API BOOL			IntersectTriangle(PIntersectData* pOutPick, const float3& vOrig, const float3& vDir, const float3& v0, const float3& v1, const float3& v2, const matrix4x4* pWorld, bool bDoubleSide)
	{CPUTime("matrix4x4");
	...
	PHANTOM_API BOOL	IntersectTriangleList(PIntersectData* pOutPick, const float3& vOrig, const float3& vDir, const void* _verts, const void* pIndices, int numFace, const matrix4x4* pWorld, int strideSize, int cull)
	{CPUTime("matrix4x4");
		float3 vPickRayOrig;
		float3 vPickRayDir;
		//
	...
	//float2
	PHANTOM_API float2*	vec2BaryCentric(float2 *pOut, const float2 *pV1, const float2 *pV2, const float2 *pV3, float f, float g)
	{
		pOut->x = (1.0f-f-g) * (pV1->x) + f * (pV2->x) + g * (pV3->x);
		pOut->y = (1.0f-f-g) * (pV1->y) + f * (pV2->y) + g * (pV3->y);
		return pOut;
	}
	PHANTOM_API float2*  vec2CatmullRom(float2 *pOut, const float2 *pV0, const float2 *pV1, const float2 *pV2, const float2 *pV3, float s)
	{
	...
	PHANTOM_API float2*  vec2Hermite(float2 *pOut, const float2 *pV1, const float2 *pT1, const float2 *pV2, const float2 *pT2, float s)
	{
		float h1, h2, h3, h4;
	...
	PHANTOM_API float4*  vec2Transform(float4 *pOut, const float2 *pV, const matrix4x4 *pMat)
	{
	...
	PHANTOM_API float4*  vec2TransformArray(float4* pOut, unsigned int outstride, const float2* pIn, unsigned int instride, const matrix4x4* pMatrix, unsigned int elements)
	{
		unsigned int i;
	...
	PHANTOM_API float2*  vec2TransformCoord(float2 *pOut, const float2 *pV, const matrix4x4 *pMat)
	{
		float2 v;
		float norm;
		v = *pV;
	...
	PHANTOM_API float2*  vec2TransformCoordArray(float2* pOut, unsigned int outstride, const float2* pIn, unsigned int instride, const matrix4x4* pMatrix, unsigned int elements)
	{
	...
	PHANTOM_API float2*  vec2TransformNormal(float2 *pOut, const float2 *pV, const matrix4x4 *pMat)
	{
		const float2 v = *pV;
	...
	PHANTOM_API float2*  vec2TransformNormalArray(float2* pOut, unsigned int outstride, const float2 *pIn, unsigned int instride, const matrix4x4 *pMatrix, unsigned int elements)
	{
		for(unsigned int i = 0; i < elements; ++i)
		{
	...
	PHANTOM_API float3*  vec3BaryCentric(float3 *pOut, const float3 *pV1, const float3 *pV2, const float3 *pV3, float f, float g)
	{
		pOut->x = (1.0f-f-g) * (pV1->x) + f * (pV2->x) + g * (pV3->x);
		pOut->y = (1.0f-f-g) * (pV1->y) + f * (pV2->y) + g * (pV3->y);
	...
	PHANTOM_API float3*  vec3CatmullRom( float3 *pOut, const float3 *pV0, const float3 *pV1, const float3 *pV2, const float3 *pV3, float s)
	{
		pOut->x = 0.5f * (2.0f * pV1->x + (pV2->x - pV0->x) *s + (2.0f *pV0->x - 5.0f * pV1->x + 4.0f * pV2->x - pV3->x) * s * s + (pV3->x -3.0f * pV2->x + 3.0f * pV1->x - pV0->x) * s * s * s);
		pOut->y = 0.5f * (2.0f * pV1->y + (pV2->y - pV0->y) *s + (2.0f *pV0->y - 5.0f * pV1->y + 4.0f * pV2->y - pV3->y) * s * s + (pV3->y -3.0f * pV2->y + 3.0f * pV1->y - pV0->y) * s * s * s);
	...
	PHANTOM_API float3*  vec3Hermite(float3 *pOut, const float3 *pV1, const float3 *pT1, const float3 *pV2, const float3 *pT2, float s)
	{
		float h1, h2, h3, h4;
	...
	PHANTOM_API float4*  vec3Transform(float4 *pOut, const float3 *pV, const matrix4x4 *pMat)
	{
	...
	PHANTOM_API float4*  vec3TransformArray(float4* pOut, unsigned int outstride, const float3* pIn, unsigned int instride, const matrix4x4* pMatrix, unsigned int elements)
	{
		unsigned int i;
	...
	PHANTOM_API float3*  vec3TransformCoord(float3 *pOut, const float3 *pV, const matrix4x4 *pMat)
	{
		float3 o;
		float norm;
	...
	PHANTOM_API float3*  vec3TransformCoordArray(float3* pOut, unsigned int outstride, const float3* pIn, unsigned int instride, const matrix4x4* pMatrix, unsigned int elements)
	{
		unsigned int i;
	...
	PHANTOM_API float3*  vec3TransformNormal(float3 *pOut, const float3 *pV, const matrix4x4 *pMat)
	{
		const float3 v = *pV;
	...
	PHANTOM_API float3*  vec3TransformNormalArray(float3* pOut, unsigned int outstride, const float3* pIn, unsigned int instride, const matrix4x4* pMatrix, unsigned int elements)
	{
		unsigned int i;
	...
	PHANTOM_API float4*  vec4BaryCentric(float4 *pOut, const float4 *pV1, const float4 *pV2, const float4 *pV3, float f, float g)
	{
		pOut->x = (1.0f-f-g) * (pV1->x) + f * (pV2->x) + g * (pV3->x);
		pOut->y = (1.0f-f-g) * (pV1->y) + f * (pV2->y) + g * (pV3->y);
		pOut->z = (1.0f-f-g) * (pV1->z) + f * (pV2->z) + g * (pV3->z);
	...
	PHANTOM_API float4*  vec4CatmullRom(float4 *pOut, const float4 *pV0, const float4 *pV1, const float4 *pV2, const float4 *pV3, float s)
	{
		pOut->x = 0.5f * (2.0f * pV1->x + (pV2->x - pV0->x) *s + (2.0f *pV0->x - 5.0f * pV1->x + 4.0f * pV2->x - pV3->x) * s * s + (pV3->x -3.0f * pV2->x + 3.0f * pV1->x - pV0->x) * s * s * s);
		pOut->y = 0.5f * (2.0f * pV1->y + (pV2->y - pV0->y) *s + (2.0f *pV0->y - 5.0f * pV1->y + 4.0f * pV2->y - pV3->y) * s * s + (pV3->y -3.0f * pV2->y + 3.0f * pV1->y - pV0->y) * s * s * s);
		pOut->z = 0.5f * (2.0f * pV1->z + (pV2->z - pV0->z) *s + (2.0f *pV0->z - 5.0f * pV1->z + 4.0f * pV2->z - pV3->z) * s * s + (pV3->z -3.0f * pV2->z + 3.0f * pV1->z - pV0->z) * s * s * s);
	...
	PHANTOM_API float4*  vec4Cross(float4 *pOut, const float4 *pV1, const float4 *pV2, const float4 *pV3)
	{
		float4 o;
	...
	PHANTOM_API float4*  vec4Hermite(float4 *pOut, const float4 *pV1, const float4 *pT1, const float4 *pV2, const float4 *pT2, float s)
	{
		float h1, h2, h3, h4;
	...
	PHANTOM_API float4*  vec4Transform(float4 *pOut, const float4 *pV, const matrix4x4 *pMat)
	{
		float4 o;
	...
	PHANTOM_API float4*  vec4TransformArray(float4* pOut, unsigned int outstride, const float4* pIn, unsigned int instride, const matrix4x4* pMatrix, unsigned int elements)
	{
		unsigned int i;
	...
	PHANTOM_API quaternion*  qtnBaryCentric(quaternion *pOut, const quaternion *pQ1, const quaternion *pQ2, const quaternion *pQ3, float f, float g)
	{
	...
	PHANTOM_API quaternion*  qtnExp(quaternion *pOut, const quaternion *pQtn)
	{
		float norm;
	...
	PHANTOM_API quaternion*  qtnInverse(quaternion *pOut, const quaternion *pQtn)
	{
		quaternion o;
		float norm;
	...
	PHANTOM_API quaternion*  qtnLn(quaternion *pOut, const quaternion *pQtn)
	{
	...
	PHANTOM_API quaternion*  qtnMultiply(quaternion *pOut, const quaternion *pQ1, const quaternion *pQ2)
	{
		quaternion o;
	...
	PHANTOM_API quaternion*  qtnNormalize(quaternion *pOut, const quaternion *pQtn)
	{
		quaternion o;
		float norm;
	...
	PHANTOM_API quaternion*  qtnRotationAxis(quaternion *pOut, const float3 *pV, float angle)
	{
		float3 temp;
	...
	PHANTOM_API quaternion*  qtnRotationMatrix(quaternion *pOut, const matrix4x4 *pMat)
	{
		int i, maxi;
	...
	PHANTOM_API quaternion*  qtnRotationYawPitchRoll(quaternion *pOut, float fYaw, float fPitch, float fRoll)
	{
	...
	PHANTOM_API quaternion*  qtnSlerp(quaternion *pOut, const quaternion *pQ1, const quaternion *pQ2, float t)
	{
	...
	PHANTOM_API quaternion*  qtnSquad(quaternion *pOut, const quaternion *pQ1, const quaternion *pQ2, const quaternion *pQ3, const quaternion *pQ4, float t)
	{
		quaternion temp1, temp2;
	...
	PHANTOM_API void  qtnToAxisAngle(const quaternion *pQtn, float3 *paxis, float *pangle)
	{
		paxis->x = pQtn->x;
		paxis->y = pQtn->y;
		paxis->z = pQtn->z;
	...
	PHANTOM_API Plane*  planeFromPoints(Plane *pOut, const float3 *pV1, const float3 *pV2, const float3 *pV3)
	{
	...
	PHANTOM_API float3*  planeIntersectLine(float3 *pOut, const Plane *pp, const float3 *pV1, const float3 *pV2)
	{
	...
	PHANTOM_API Plane*  planeNormalize(Plane *pOut, const Plane *pp)
	{
		Plane o;
		float norm;
	...
	PHANTOM_API Plane*  planeTransform(Plane *pOut, const Plane *pplane, const matrix4x4 *pMat)
	{
	...
	PHANTOM_API Plane*  planeTransformArray(Plane* pOut, unsigned int outstride, const Plane* pIn, unsigned int instride, const matrix4x4* pMatrix, unsigned int elements)
	{
		unsigned int i;
	...
	PHANTOM_API matrix4x4*  matrixAffineTransformation(matrix4x4 *pOut, float scaling, const float3 *rotationcenter, const quaternion *rotation, const float3 *translation)
	{
	...
	PHANTOM_API matrix4x4*  matrixAffineTransformation2D(matrix4x4 *pOut, float scaling, const float2 *protationcenter, float rotation, const float2 *ptranslation)
	{
		matrix4x4 m1, m2, m3, m4, m5;
		quaternion rot;
	...
	PHANTOM_API BOOL  matrixDecompose(float3 *poutscale, quaternion *poutrotation, float3 *pouttranslation, const matrix4x4 *pMat)
	{
	...
	PHANTOM_API matrix4x4*  matrixLookAtLH(matrix4x4 *pOut, const float3 *peye, const float3 *pat, const float3 *pup)
	{CPUTime("matrix4x4");
	...
	PHANTOM_API matrix4x4*  matrixMultiplyTranspose(matrix4x4 *pOut, const matrix4x4 *pm1, const matrix4x4 *pm2)
	{
	...
	PHANTOM_API matrix4x4*  matrixOrthoLH(matrix4x4 *pOut, float w, float h, float zn, float zf)
	{CPUTime("matrix4x4");
		matrixIdentity(pOut);
		pOut->m[0][0] = 2.0f / w;
		pOut->m[1][1] = 2.0f / h;
	...
	PHANTOM_API matrix4x4*  matrixOrthoOffCenterLH(matrix4x4 *pOut, float l, float r, float b, float t, float zn, float zf)
	{
		matrixIdentity(pOut);
		pOut->m[0][0] = 2.0f / (r - l);
		pOut->m[1][1] = 2.0f / (t - b);
	...
	PHANTOM_API matrix4x4*  matrixOrthoOffCenterRH(matrix4x4 *pOut, float l, float r, float b, float t, float zn, float zf)
	{
		matrixIdentity(pOut);
		pOut->m[0][0] = 2.0f / (r - l);
		pOut->m[1][1] = 2.0f / (t - b);
	...
	PHANTOM_API matrix4x4*  matrixPerspectiveFovLH(matrix4x4 *pOut, float fovy, float aspect, float zn, float zf)
	{
		matrixIdentity(pOut);
	...
	PHANTOM_API matrix4x4*  matrixPerspectiveLH(matrix4x4 *pOut, float w, float h, float zn, float zf)
	{
		matrixIdentity(pOut);
	...
	PHANTOM_API matrix4x4*  matrixPerspectiveOffCenterLH(matrix4x4 *pOut, float l, float r, float b, float t, float zn, float zf)
	{
		matrixIdentity(pOut);
	...
	PHANTOM_API matrix4x4*  matrixPerspectiveOffCenterRH(matrix4x4 *pOut, float l, float r, float b, float t, float zn, float zf)
	{
		matrixIdentity(pOut);
		pOut->m[0][0] = 2.0f * zn / (r - l);
		pOut->m[1][1] = -2.0f * zn / (b - t);
	...
	PHANTOM_API matrix4x4*  matrixPerspectiveRH(matrix4x4 *pOut, float w, float h, float zn, float zf)
	{
		matrixIdentity(pOut);
		pOut->m[0][0] = 2.0f * zn / w;
		pOut->m[1][1] = 2.0f * zn / h;
	...
	PHANTOM_API matrix4x4*  matrixReflect(matrix4x4 *pOut, const Plane *pplane)
	{
		Plane np;
	...
	PHANTOM_API matrix4x4*  matrixRotationAxis(matrix4x4 *pOut, const float3 *pV, float angle)
	{
		float3 v;
	...
	PHANTOM_API matrix4x4*  matrixRotationQuaternion(matrix4x4 *pOut, const quaternion *pq)
	{
		matrixIdentity(pOut);
	...
	PHANTOM_API matrix4x4*  matrixRotationYawPitchRoll(matrix4x4 *pOut, float fYaw, float fPitch, float fRoll)
	{
		matrix4x4 m;
	...
	PHANTOM_API matrix4x4*  matrixShadow(matrix4x4 *pOut, const float4 *plight, const Plane *pplane)
	{
		Plane np;
		float dot;
	...
	PHANTOM_API matrix4x4*  matrixTransformation(matrix4x4 *pOut, const float3 *pscalingcenter, const quaternion *pscalingrotation, const float3 *pscaling, const float3 *protationcenter, const quaternion *protation, const float3 *ptranslation)
	{
	...
	PHANTOM_API matrix4x4*  matrixTransformation2D(matrix4x4 *pOut, const float2 *pscalingcenter, float scalingrotation, const float2 *pscaling, const float2 *protationcenter, float rotation, const float2 *ptranslation)
	{
	...
	PHANTOM_API matrix4x4*  matrixTranslation(matrix4x4 *pOut, float x, float y, float z)
	{
		matrixIdentity(pOut);
		pOut->m[3][0] = x;
		pOut->m[3][1] = y;
	...
	PHANTOM_API matrix4x4*  matrixTranspose(matrix4x4 *pOut, const matrix4x4 *pMat)
	{
	...
	color4::color4(const char* text) {
		TextSection s[3];
		int num = SplitText(text, s, 3, ',');
		if (num == 1) {
			r = g = b = a = (float)atoi(s[0].text) / 255.0f;
		}
		else if (num == 3) {
			r = (float)atoi(s[0].text) / 255.0f;
			g = (float)atoi(s[1].text) / 255.0f;
			b = (float)atoi(s[2].text) / 255.0f;
			a = 1.0f;
		}
		else if (num == 4) {
			r = (float)atoi(s[0].text) / 255.0f;
	...
	PHANTOM_API int	SplitText(const char* szText, TextSection* secs, int secCount, char symbol, bool bRemoveSpace) {
		int last = 0;
		int secindex = 0;
		int writeindex = 0;
		while (true) {
			...
			if (ch == symbol || ch == 0)
			{
				secs[secindex].text[writeindex] = 0;
				secindex++;
				if (secindex >= secCount || ch == 0)
					return secindex;
				writeindex = 0;
			}
			else {
				if (!(bRemoveSpace&&ch == ' '))
					secs[secindex].text[writeindex++] = ch;
			}
			szText++;
		}
		return secindex;
