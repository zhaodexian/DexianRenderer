...
namespace Phantom{
	...
	BOOL			Manager::DrawTexts(RenderText* textPtr, BOOL bEnabled, Rect& drr, unsigned int dtFormat, const color4& c, const matrix4x4* world, int nHighlight, bool bWithBack)
	{CPUTimeT(Manager);
		...
		if (bWithBack) {
			if (!SetRenderMode(RenderMode_TextWithBack))
				return false;
		}
		else {
...
VOID			Manager::DrawLines2D(const Pixel* xys, int count, const color4& c, BOOL bFill/*是否填充*/)	//渲染2D线条
	...
	void		Manager::RenderTexture(DrawBuffer* buffer, Texture* tex, const RectF& rc, const RectF& texRc, const color4& c, bool bEnabled)
	{
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		m_device->SetAlphaMode(Phantom::AlphaMode_Blend);
		...
		if (bEnabled) {
			...
			if (!SetRenderMode(RenderMode_ImageBufferDisable))
				return;
		}
		m_device->SetCullVisible(CullMode_CW);
		if (!buffer->Valid || m_bSizeChanged) {
	...
	VOID		Manager::RenderText2D(DrawBuffer* buffer, RenderText* textPtr, const RectF& rc, const color4& c, const RectF* TextRect, bool bBack)
	{
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		...
		else if (!SetRenderMode(RenderMode_TextBuffer))
			return;
		if (!buffer->Valid || m_bSizeChanged) {
			Rect textRect = textPtr->GetTextureRect();
			if (TextRect)
	...
	BOOL			Manager::DrawTexts(const char* szUTF8, Rect& destRect, int nLength, float fontSize, float fItalic, const color4* c, unsigned int dtFormat, const matrix4x4* world, bool bWithBack)
	{
		CPUTimeNone();
		if (!m_tempText) {
        ...
        color4 temp(1,1,1,1);
        if(!c)c=&temp;
        return DrawTexts(m_tempText, true, destRect, dtFormat, *c, world, 0, bWithBack);
	}
