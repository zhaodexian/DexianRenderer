...
{CPUTime(AtomicLine);
	TypeArray<AotmicData*>&		vAtomic = pNode->m_vNodeActive;
...
static TypeArray<unsigned int>	g_tempColors;
...
void AtomicLine::Draw(TypeArray<AotmicData*>& vAtomic, EffectNode* node)
{CPUTime(AtomicLine);
	...
	if( !m_line.m_bLighting )
	{
		static TypeArray<color4> colors;
		static TypeArray<float3> verts;
		static TypeArray<float2> uvs;
		colors.resize(vAtomic.size() * 2);
		verts.resize(vAtomic.size() * 2);
		uvs.resize(vAtomic.size() * 2);
		int nIndex = 0;
...
void AtomicMesh::Draw(TypeArray<AotmicData*>& vAtomic, EffectNode* node)
{CPUTime(AtomicMesh);
	AotmicData*	pAtomic;
	float3*	pArrayVer;
	int*		pTexIndex	= (int*)m_meshPtr->m_pFrame->m_pTexID[0];
...
void AtomicPlane::Draw(TypeArray<AotmicData*>& vAtomic, EffectNode* node)
{CPUTime(AtomicPlane);
	AotmicData*	pAtomic;
	SProperty* pPro = &node->m_pPro;
	if( pPro->bAtomicInstTwoSide )
